...
public class PrimaryIndexingController {
    ...
    @ApiResponse(response = PrimarySuccessResponse.class, code = 200, message = "sucess")
    @Consumes("application/json")
    @Produces("application/json")
    @Transactional
    public ResponseEntity<PrimarySuccessResponse> index(@RequestBody PrimaryRequestBody[] request) {
        if (request != null && request.length != 0) {
            ...
            if (exist == null) {
                ...
                return ResponseEntity.status(HttpStatus.OK).body(PrimarySuccessResponse.create());
            }
            return ResponseEntity.status(HttpStatus.OK).body(PrimarySuccessResponse.create());
        } else
            return ResponseEntity.status(HttpStatus.OK).body(PrimarySuccessResponse.create());
    ...
    @Produces("application/json")
    @Transactional
    public ResponseEntity<PrimarySuccessResponse> sigin(@RequestBody RequestCreateUser[] request) {
        if (request != null && request.length != 0) {
            phone = request[0].getPhone();
    ...
    @Produces("application/json")
    @Transactional
    public ResponseEntity<PrimarySuccessResponse> selectUserName(@RequestBody SuccessLogin[] login) {
